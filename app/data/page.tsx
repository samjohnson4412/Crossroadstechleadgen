...
const SOURCES = [
  {
    ...
    description: 'Pulls the official daily filing files from the Florida Division of Corporations data server and keeps the new Tampa Bay LLCs and corporations. These are businesses that need everything: VoIP, email, M365, Wi-Fi, cameras.',
...
const FCC_LOOKUP_INFO = {
  label: 'FCC Broadband Coverage Lookup',
  description: 'Use this on any lead\'s detail page to verify the address, auto-fill the county, and jump straight to the FCC National Broadband Map to see which ISPs serve that location.',
  icon: <Wifi size={20} />,
  color: 'var(--cyan)',
}
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
          Automated lead ingestion from public Tampa Bay data sources
        </p>
      </div>
      ...
      <div className="space-y-4">
            ...
            <div key={source.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-white" style={{ background: source.color }}>
                    {source.icon}
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-4 mt-3">
                      <span className="text-xs px-2 py-1 rounded-full border border-gray-200" style={{ color: 'var(--text-muted)' }}>
                        Pitch: {source.pitch}
                      </span>
                      {lastRun && (
                        <span className="text-xs flex items-center gap-1" style={{ color: 'var(--text-muted)' }}>
                          ...
                          {' · '}
                          {lastRun.records_inserted} inserted
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <div className="flex items-center gap-1">
                    ...
                    <input
                      ...
                      min={1}
                      max={365}
                      value={daysBack[source.id] ?? source.defaultDays}
                    ...
                    <details className="mt-2">
                      ...
                      <ul className="mt-1 space-y-1">
                        {result.errors.slice(0, 5).map((e, i) => (
                          <li key={i} className="text-xs" style={{ color: 'var(--danger)' }}>{e}</li>
                        ))}
                      </ul>
                    </details>
                  )}
        ...
        {/* FCC info card (not triggerable — used from lead detail) */}
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'var(--cyan)' }}>
              <span style={{ color: 'var(--navy)' }}>{FCC_LOOKUP_INFO.icon}</span>
            </div>
            <div>
              <h2 className="font-semibold" style={{ color: 'var(--navy)' }}>{FCC_LOOKUP_INFO.label}</h2>
              <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>{FCC_LOOKUP_INFO.description}</p>
              <div className="mt-3">
                <span className="text-xs px-2 py-1 rounded-full border border-gray-200" style={{ color: 'var(--text-muted)' }}>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        ...
        <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
          Add this to <code className="bg-gray-100 px-1 py-0.5 rounded text-xs">vercel.json</code> to run ingestion automatically on a schedule:
        </p>
        <pre className="bg-gray-900 text-green-400 rounded-xl p-4 text-xs overflow-x-auto">
{`{
  "crons": [
    {
      "path": "/api/ingest/sunbiz",
      "schedule": "0 8 * * 1"
    },
    {
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={loadLogs} className="text-xs flex items-center gap-1" style={{ color: 'var(--blue)' }}>
                  ...
                  <div className="flex items-center gap-4">
                    {log.status === 'success' && (
                      <div className="text-xs flex gap-3" style={{ color: 'var(--text-muted)' }}>
                        <span className="text-green-700 font-medium">+{log.records_inserted}</span>
                        <span>{log.records_skipped} skipped</span>
                      </div>
                    )}
                    {log.status === 'error' && (
